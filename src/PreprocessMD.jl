...
"""
function add_label_column!(target_df::AbstractDataFrame, new_col_name::Symbol, label_df::AbstractDataFrame, label_symb_target::Symbol, label_symb_label::Symbol)::Nothing
	insertcols!(target_df, new_col_name => [x[label_symb_target] in label_df[!,label_symb_label] for x in eachrow(target_df)])
	coerce!(target_df, new_col_name => OrderedFactor{2})
	return nothing
end
